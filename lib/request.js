...
class Request {
  constructor(hostname, { headers = { }, failOnLimitReached = false, agent = null, maxRetries = 10 } = { }) {
  ...
  run(method, path, data, noOfAttempts = 0) {
    ...
    return new Promise((resolve, reject) => {
      const req = https.request(options, res => {
        ...
        if (res.statusCode === 429) {
          const timeToWait = res.headers['x-retry-after'] || backOffTime(noOfAttempts || 1);

          if (this.failOnLimitReached ||  noOfAttempts >= (this.maxRetries - 1)) {
          ...
          return setTimeout(() => {
            ...
            this.run(method, path, data, ++noOfAttempts)
              .then(resolve)
              .catch(reject);
          }, timeToWait * 1000);
        }
...
function backOffTime(numFailures) {
